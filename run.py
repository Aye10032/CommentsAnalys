...
def load_data(data_path: str, init: bool = False) -> Tuple[DataFrame, list]:
    if init:
        ...
        comment_df.to_csv('data/news.csv', encoding='utf-8', index=False)

        opi_list = []
    else:
        comment_df = pd.read_csv('data/news.csv', encoding='utf-8')
        with open('data/ops.txt', 'r', encoding='utf-8') as op_f:
...
def main() -> None:
    ...
    data, lists = load_data('data/news.txt', init)
    ...
    for index, row in tqdm(data.iterrows(), total=30):
        ...
        with open('data/ops.txt', 'w', encoding='utf-8') as op_f:
            op_f.write('\n'.join(lists))
        data.at[index, 'generate'] = True
        data.to_csv('data/news.csv', encoding='utf-8', index=False)

    if init or not os.path.exists('data/result.csv'):
